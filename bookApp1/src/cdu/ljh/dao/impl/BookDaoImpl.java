...
public class BookDaoImpl extends BaseDao implements BookDao {
    ...
    public int insert(Book book) {
        int rows = 0;
        String sql = "INSERT INTO book_table(title,author,press,price,sale,stock,coverUrl,info,publishDate," +
                "marketDate) VALUES(?,?,?,?,?,?,?,?,?,?) ";
        try {
            pstmt = conn.prepareStatement(sql);
    ...
    public int update(Book book) {
        int rows = 0;
        String sql = "UPDATE book_table SET title=?,author=?,press=?,price=?,sale=?,stock=?,coverUrl=?,info=?," +
                "publishDate=?,marketDate=? WHERE id=?";
        try {
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, book.getTitle());
            pstmt.setString(2, book.getAuthor());
